...
<title>Mon Profil Admin — Jokko Santé</title>
...
.hero-role{display:inline-block;background:rgba(255,215,0,.2);color:#ffd700;padding:4px 14px;border-radius:50px;font-size:.78rem;font-weight:700;}
...
.stat-item{background:white;border-radius:14px;padding:18px;text-align:center;border:1px solid rgba(0,0,0,.06);border-top:3px solid var(--forest);}
...
.quick-links{display:grid;grid-template-columns:repeat(3,1fr);gap:14px;margin-bottom:24px;}
.quick-link{background:white;border-radius:16px;border:1px solid rgba(0,0,0,.07);padding:18px;text-decoration:none;display:flex;align-items:center;gap:12px;transition:all .3s;}
.quick-link:hover{transform:translateY(-4px);box-shadow:0 12px 32px rgba(26,61,22,.1);border-color:var(--light);}
.quick-link-icon{width:40px;height:40px;border-radius:12px;display:flex;align-items:center;justify-content:center;font-size:1.2rem;flex-shrink:0;}
.quick-link h4{font-size:.88rem;font-weight:600;color:var(--dark);margin:0 0 2px;}
.quick-link p{font-size:.75rem;color:var(--muted);margin:0;}
</style>
</head>
<body>
...
<nav class="navbar">
    <div class="container d-flex align-items-center justify-content-between">
        ...
        <a href="/dashboard" class="btn-nav">← Dashboard Admin</a>
    </div>
</nav>
    ...
    <div class="container">
        <div class="hero-inner">
            ...
            <div>
                ...
                <span class="hero-role">👑 Administrateur</span>
                <div class="hero-info">
...
<div class="main">
    ...
    <div class="stats-grid">
        ...
        <div class="stat-item" style="border-top-color:#3b82f6;">
            <div class="stat-num" style="color:#3b82f6;">{{ $stats['total_articles'] }}</div>
            <div class="stat-lbl">📝 Articles</div>
        </div>
        <div class="stat-item" style="border-top-color:#a855f7;">
            <div class="stat-num" style="color:#a855f7;">{{ $stats['total_rdv'] }}</div>
            <div class="stat-lbl">📅 Rendez-vous</div>
        </div>
        <div class="stat-item" style="border-top-color:#f59e0b;">
            <div class="stat-num" style="color:#f59e0b;">{{ $stats['total_forums'] }}</div>
            <div class="stat-lbl">💬 Forums</div>
        </div>
    </div>

    {{-- ACCÈS RAPIDES ADMIN --}}
    <p style="font-size:.75rem;font-weight:700;letter-spacing:.14em;text-transform:uppercase;color:var(--muted);margin-bottom:14px;">Accès rapides</p>
    <div class="quick-links">
        <a href="/admin/utilisateurs" class="quick-link">
            <div class="quick-link-icon" style="background:var(--light);">👥</div>
            <div><h4>Utilisateurs</h4><p>Gérer les rôles</p></div>
        </a>
        <a href="/admin/articles" class="quick-link">
            <div class="quick-link-icon" style="background:#dbeafe;">📝</div>
            <div><h4>Articles</h4><p>Gérer le contenu</p></div>
        </a>
        <a href="/admin/rendezvous" class="quick-link">
            <div class="quick-link-icon" style="background:#fef3c7;">📅</div>
            <div><h4>Rendez-vous</h4><p>Suivre les RDV</p></div>
        </a>
    </div>
